...
Run from the workspace root with:
    python -m Playground_Kai.plot_results_kai
    python -m Playground_Kai.plot_results_kai --model CONFORMER
"""
...
def _load_summary(model: str, filename: str) -> pd.DataFrame | None:
...
def _load_curves(model: str, filename: str, run_ids: list[str] | None = None) -> pd.DataFrame | None:
    path = os.path.join(RESULTS_DIR, filename)
    if not os.path.exists(path):
...
def main() -> None:
    ...
    # ---------------------------------------------------------------------------
    # CSV filenames — edit these to point at the files you want to plot.
    # Both files must live in RESULTS_DIR.
    # ---------------------------------------------------------------------------
    summary_csv = "kai_results_summary_CONFORMER_fullraw.csv"
    curves_csv  = "kai_results_curves_CONFORMER_fullraw.csv"

    summary_csv = "results_summary_CONFORMER.csv"
    curves_csv  = "results_curves_CONFORMER.csv"


    summary = _load_summary(model, summary_csv)
    ...
    curves = _load_curves(model, curves_csv, run_ids=summary["run_id"].tolist())
