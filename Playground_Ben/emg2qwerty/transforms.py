...
@dataclass
class ChannelSelect:
    """Keeps only the electrode channels at the given indices from each band.
    Expects input of shape (T, bands, C, freq) — i.e. after ``LogSpectrogram``.

    Args:
        indices (list[int]): Channel indices to keep (0-based).

    Example — every other channel:
        ChannelSelect(indices=[0, 2, 4, 6, 8, 10, 12, 14])
    """

    indices: Sequence[int]

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        # tensor: (T, bands, C, freq)
...
@dataclass
class ChannelSubset:
    """Keeps only the first ``num_channels`` electrode channels from each band.
    Expects input of shape (T, bands, C, freq) — i.e. after ``LogSpectrogram``.

    Args:
        num_channels (int): Number of channels to keep (must be <= 16).
    """
...
class BandpassFilter:
    """Applies a zero-phase FIR bandpass filter to the raw EMG signal.
    Apply before ``TemporalDownsample`` and ``LogSpectrogram``.
    Expects input of shape (T, ...).

    Designed for EMG preprocessing: removes DC drift and low-frequency
    motion artefacts (below ``low_hz``) and out-of-band noise / aliasing
    components above ``high_hz``.  Defaults of 20–460 Hz are appropriate
    for a 2000 Hz sEMG signal where PSD analysis shows >99% of signal
    energy lies below 460 Hz.

    Uses a Hamming-windowed sinc FIR filter applied via FFT convolution
    (torch.nn.functional.conv1d) for efficiency.

    Args:
        low_hz  (float): Lower cutoff frequency in Hz. (default: 20.0)
        high_hz (float): Upper cutoff frequency in Hz. (default: 460.0)
        sample_rate (int): Sample rate of the raw EMG in Hz. (default: 2000)
        num_taps (int): FIR filter length (odd integer). Longer filters give
            sharper roll-off but more latency. (default: 101)
    """
    ...
    def __post_init__(self) -> None:
        assert self.num_taps % 2 == 1, "num_taps must be odd"
        # Build Hamming-windowed sinc bandpass kernel (1-D, shape: num_taps)
        n = torch.arange(self.num_taps) - (self.num_taps - 1) / 2
        low  = self.low_hz  / self.sample_rate
        high = self.high_hz / self.sample_rate
        # sinc bandpass = high-pass sinc - low-pass sinc (both normalised to Nyquist=1)
        h = (2 * high * torch.sinc(2 * high * n)
             - 2 * low  * torch.sinc(2 * low  * n))
        h = h * torch.hamming_window(self.num_taps, periodic=False)
        h = h / h.sum()  # unity gain at DC (normalise)
        # Store as (1, 1, num_taps) conv1d weight — applied channel-by-channel
        self._kernel: torch.Tensor = h.view(1, 1, -1)

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        # tensor: (T, ...) — flatten all non-time dims, filter, restore
        T = tensor.shape[0]
        x = tensor.reshape(T, -1).float()   # (T, C_flat)
        x = x.permute(1, 0).unsqueeze(1)    # (C_flat, 1, T)

        kernel = self._kernel.to(x.device)
        pad = (self.num_taps - 1) // 2

        # Forward pass
        y = torch.nn.functional.conv1d(x, kernel, padding=pad)
        # Backward pass (zero-phase / zero-delay filtering)
        y = torch.flip(y, dims=[-1])
        y = torch.nn.functional.conv1d(y, kernel, padding=pad)
        y = torch.flip(y, dims=[-1])

        y = y.squeeze(1).permute(1, 0)      # (T, C_flat)
        return y.reshape(tensor.shape).to(tensor.dtype)


@dataclass
class TemporalDownsample:
    """Downsamples the raw EMG signal along the time axis using proper
    anti-aliased resampling (torchaudio.functional.resample).  Apply before
    ``LogSpectrogram``.  Expects input of shape (T, ...).

    Uses a polyphase anti-aliasing filter internally, so no spectral aliasing
    occurs — equivalent to recording at a lower physical sampling rate.

    Reduces the effective sample rate: with the default EMG rate of 2000 Hz,
    factor=2 → 1000 Hz, factor=4 → 500 Hz, factor=8 → 250 Hz, factor=16 → 125 Hz.

    Args:
        factor (int): Integer downsampling factor (must be >= 1).
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        if self.factor == 1:
            return tensor
        # tensor: (T, ...) — resample expects (..., T), so move time to last dim
        x = tensor.movedim(0, -1)                                    # (..., T)
        x = torchaudio.functional.resample(x, orig_freq=self.factor, new_freq=1)
        return x.movedim(-1, 0)                                      # (T', ...)
